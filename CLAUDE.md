...
- Keep the `sidebar` array directly in `astro.config.mjs` — do not extract it to a separate file, as Astro's dev server does not watch imported config modules for hot reload
