...
This repo holds **only content**. The Astro Starlight template, sidebar config, and build pipeline live in [OzzyCzech/starlight](https://github.com/OzzyCzech/starlight), which is invoked by `.github/workflows/deploy.yml` on every push.
...
- When referencing pages by slug in the `sidebar` array in `starlight.config.mjs`, always use lowercase paths (e.g. `'ai/benchmarks'`, not `'AI/Benchmarks'`) — Starlight slugs are always lowercase regardless of file casing
...
- Never add redirects to `starlight.config.mjs` when reorganizing content — only do so when explicitly asked