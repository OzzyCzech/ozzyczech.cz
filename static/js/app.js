...
hotkeys('cmd+k,esc', (event, handler) => {
...
search.addEventListener('keyup',
	(event) => {
		...
		if (search.value.startsWith('#')) {
			results = fuse.search({tags: '=' + search.value});
		} else {
			results = fuse.search({title: search.value});
		}
		...
		for (const result of results) {
			res.push({title: result.item.title, tags: result.item.tags});
		}
