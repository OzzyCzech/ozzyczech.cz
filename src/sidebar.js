export function getSidebarMenu(pages, active, className = '') {
	...
	for (const page of pages) {
		if (page?.children) {
			...
			menu += `<li>
				<button class="flex justify-between w-full px-2.5 py-1.5 rounded font-semibold hover:bg-gray-200 dark:hover:bg-gray-800 ${expanded ? 'text-yellow-300' : ''}" aria-expanded="${expanded}">
					<span>${page.name}</span>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" height="24px" width="24px" class="fill-natural-500 dark:fill-gray-400">
			...
			menu += `<li>
								<a href="${page.slug}" ${page.slug === active ? 'aria-current="page"' : ''} class="flex px-2.5 py-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-800 w-full ${page.slug == active ? 'dark:bg-gray-800 text-yellow-300' : ''}">
									${page.name}
								</a>
							</li>`;
		}
	...
	return `
			<nav class="font-semibold">
				<div class="mx-2 mt-2 mb-1">
					<a href="/" class="flex justify-between px-2.5 py-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-800">
						<span>Homepage</span>
