export function getSidebarMenu(pages, active, className = '') {
  ...
  for (const page of pages) {
    if (page?.children) {
      ...
      menu += `<li>
				<button class="flex justify-between w-full px-3 py-2 rounded font-semibold hover:bg-neutral-200 dark:hover:bg-neutral-900 ${expanded ? 'text-yellow-300' : ''}" aria-expanded="${expanded}">
					<span>${page.name}</span>
      ...
      menu += `<li>
								<a href="${page.slug}" ${page.slug === active ? 'aria-current="page"' : ''} class="flex px-3 py-2 rounded hover:bg-neutral-200 dark:hover:bg-neutral-900 w-full ${page.slug == active ? 'dark:bg-neutral-900 text-yellow-300' : ''}">
									${page.name}
								</a>
							</li>`;
    }
