export function getSidebarMenu(pages, active, className = '') {
  ...
  for (const page of pages) {
    if (page?.children) {
      ...
      menu += `<li>
				<button type="button" class="group peer flex justify-between py-1 w-full text-gray-700 dark:text-gray-200 font-semibold hover:text-blue-600 dark:hover:text-sky-500 transition" aria-expanded="${expanded}">
					<span>${page.name}</span>
      ...
      menu += `<li>
	      <a href="${page.slug}" ${page.slug === active ? 'aria-current="page"' : ''}
  		    class="flex py-0.5 w-full dark:hover:text-sky-500 hover:text-blue-700 transition ${page.slug == active ? 'text-blue-700 dark:text-sky-500' : ''}">
    		${page.name}
    	</a>
      </li>`;
    }
