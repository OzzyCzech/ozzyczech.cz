export function getSidebarMenu(pages, active, className = '') {
	...
	for (const page of pages) {
		if (page?.children) {
			...
			menu += `<li>
				<button class="flex justify-between w-full px-2.5 py-1.5 rounded font-semibold hover:bg-gray-200 dark:hover:bg-gray-800" aria-expanded="${expanded}">
					<span>${page.name}</span>
...
export function getAside(pages, active) {
	return `
			<nav class="font-semibold">
				<a href="/" class="flex justify-between w-full px-2.5 py-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-800">
					<span>Homepage</span>
